...
> ※ 2026-08-07 時点の設計メモです。現状（公開済み・USB 対応済み）とは異なる箇所があります。
...
navigator.bluetooth.requestDevice({
  filters: [
    { services: [0x180f] },        // battery — ZMK は必ず載せる
    { namePrefix: "torabo" },      // 名前も必ず載る（分割の "L-torabo-tsuki" も拾う）
...
- filters は**アドバタイズパケットの中身にしかマッチしない**。接続後に見えるサービスは
  対象外なので、`{ services: [LIVE_FEED_SERVICE] }` 単体では何も出てこない
  （ZMK の advertisement は appearance / flags と 16bit UUID 2 つだけで、128bit の
  live_feed / RPC は載らない）。載る保証があるのは battery と名前の 2 つ。
- filters は OR。取りこぼしに備えて `acceptAllDevices: true` の経路
  （「BLE（すべての機器）」ボタン → `connectBluetooth({ allDevices: true })`）を残すこと。
- 「接続済みのキーボードは advertising しない＝どうやってもダイアログに出ない」のは
  フィルタの有無と無関係。空きプロファイルへ切り替える手順が要るのはそのため。
