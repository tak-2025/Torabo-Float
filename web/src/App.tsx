...
import { errText, on as linkOn } from "./events";
import {
  canReconnect,
  close as linkClose,
  connectBluetooth,
...
export function App() {
  ...
  // Which transport the current (or last attempted) link uses. Drives the
  // route-dependent wording below: a BLE sync is minutes-slow over INDICATE,
  // a USB one is not.
  const [route, setRoute] = useState<Transport | null>(null);
  ...
  const openLink = useCallback(
    async (open: () => Promise<{ label: string }>) => {
      ...
      try {
        ...
        // Dismissing the chooser is a normal outcome, not an error worth
        // showing. Web Bluetooth's requestDevice() rejects with NotFoundError
        // ("User cancelled the requestDevice() chooser"); Web Serial's
        // requestPort() rejects with NotFoundError too, but its message is
        // "No port selected by the user" — hence both spellings below.
        const msg = errText(e);
        const cancelled =
          /User cancelled|chooser|NotFoundError: User/i.test(msg) ||
          /No port selected|No device selected/i.test(msg);
        if (!cancelled) setError(msg);
        // The link may already be open and only the subscribe have failed (a
        // firmware without tunnel support, say). Over USB that would keep the
        // port held — locking Torabo Studio out of a keyboard we are not even
        // connected to — so release whatever was opened before giving up.
        if (currentTransport()) await linkClose().catch(() => {});
        setRoute(null);
  ...
  const doConnect = () => openLink(() => connectBluetooth());
  // The filtered chooser matches only what the firmware happens to advertise,
  // so there is always a way to see everything.
  const doConnectAll = () => openLink(() => connectBluetooth({ allDevices: true }));
  const doReconnect = () => openLink(reconnectBluetooth);
  const doConnectSerial = () => openLink(connectSerial);

  const doDisconnect = async () => {
    setConn("disconnected");
    setError("");
    setRoute(null);
    await linkClose().catch(() => {});
  };
  ...
  useEffect(() => {
    ...
    return linkOn("live_feed_event", (bytes) => {
  ...
  return (
    <div className="app">
      ...
      {syncing && (
        <div className="note note-busy">
          ...
          <span className="muted">
            {route === "usb"
              ? "USB 経由なので数秒で終わります。うまくいかないときは JSON インポートが確実です。"
              : "BLE 経由なので数十秒かかることがあります。うまくいかないときは JSON インポートが確実です。"}
          </span>
        </div>
      )}
        ...
        <div className="controls">
          <div className="row">
            <button
              ...
              title="キーボードだけを一覧するダイアログを開きます"
            >
              {conn === "connecting" ? "接続中…" : "BLE で接続"}
            </button>
            <button
              onClick={doConnectSerial}
              disabled={conn === "connecting" || !isSupported("usb")}
              title={
                isSupported("usb")
                  ? "USB ケーブルで接続します（プロファイル切り替え不要）"
            ...
            <button
              className="link"
              onClick={doConnectAll}
              disabled={conn === "connecting" || !isSupported("ble")}
              title="ダイアログにキーボードが出てこないときに使います（すべての Bluetooth 機器を一覧します）"
            >
              BLE（すべての機器）
            </button>
      ...
      <main className="body">
        {showLanding ? (
          <Landing
            supported={isSupported("ble")}
            serialSupported={isSupported("usb")}
            connState={conn}
            route={route}
            canReconnect={canReconnect()}
            onConnect={doConnect}
            onConnectAll={doConnectAll}
            onConnectSerial={doConnectSerial}
