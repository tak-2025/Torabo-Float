...
export function Landing({
  ...
  supported: boolean;
  /** Web Serial available (Chrome / Edge desktop) — the USB route. */
  serialSupported: boolean;
  connState: "disconnected" | "connecting" | "connected";
  /** Which transport the live link uses, for route-dependent wording. */
  route: "ble" | "usb" | null;
  canReconnect: boolean;
  onConnect: () => void;
  /** Same, but with the chooser widened from keyboards to every device. */
  onConnectAll: () => void;
    ...
    <div className="landing">
      <section className="lp-hero">
        ...
        <p className="lp-sub muted">
          ...
          （USB は Web Serial、無線は Web Bluetooth を使います）。iOS / iPadOS の
          Safari、Firefox、macOS の Safari では動きません。キーマップの編集機能は
          ありません（表示専用）。
        </p>
        {!supported && !serialSupported && (
          <div className="error">
            このブラウザは Web Serial にも Web Bluetooth にも対応していません。
            Chrome または Edge のデスクトップ版で開いてください。
          </div>
        )}
      </section>
      ...
      <section className="lp-section">
        ...
        <p className="lp-note">
          つなぎ方は 2 通りあります。
          <strong>USB ケーブルがあるなら USB のほうが簡単</strong>です
          （下の BLE で必要なプロファイル切り替えが要りません）。
        </p>

        <div className="lp-cards">
          <div className="lp-card lp-card-hero">
            <div className="lp-card-title">
              USB でつなぐ <span className="lp-badge">かんたん</span>
            </div>
            <p className="lp-card-body">
              キーボードを USB ケーブルで PC につなぎ、下のボタンを押して
              ブラウザのダイアログでシリアルポートを選ぶだけです。
              <strong>BLE プロファイルの切り替えは不要</strong>、
              プロファイルの枠も使いません。キーマップ同期も USB のほうが高速です。
            </p>
            <ul className="lp-limits lp-tight">
              <li>
                <strong>トンネル対応のファームウェアが必要です。</strong>
                対応していない場合は「この機能にファームウェアが対応していません
                （UNSUPPORTED_FEATURE）」と表示されます。その場合は BLE
                でつないでください。
              </li>
              <li>
            ...
            <button
              ...
              disabled={!serialSupported || connecting || connected}
            >
              {connected && route === "usb"
                ? "接続済み（USB）"
                : connecting
          ...
          <div className="lp-card">
            <div className="lp-card-title">BLE（無線）でつなぐ</div>
            <p className="lp-card-body">
              ブラウザのデバイス選択ダイアログには
            ...
            <ol className="lp-steps">
              <li>
                ...
                <span className="muted">
                  （torabo-tsuki の既定ではレイヤー3 の{" "}
                  <code>&amp;bt BT_SEL 0…4</code>。
                  機種・キーマップにより異なります。BLE プロファイルを 1 枠使います）
                </span>
              </li>
              <li>
                下の<strong>「BLE で接続」</strong>を押し、ブラウザのダイアログで
                キーボードを選ぶ
              </li>
              <li>
                接続できたら、キーボードを
            ...
            <p className="lp-card-body muted">
              ダイアログには<strong>キーボードだけ</strong>を出すようにしています。
              目当てのキーボードが出てこないときは
              <strong>「BLE（すべての機器）」</strong>から、
              周囲のすべての Bluetooth 機器を一覧できます。
            </p>
            {!supported && (
              <div className="lp-warn">
          ...
          <div className="lp-card">
            ...
            <p className="lp-card-body">
              ...
              {route === "usb" ? (
                <>
                  {" "}
                  USB 接続なら<strong>数秒で終わります</strong>。
                </>
              ) : (
                <>
                  {" "}
      ...
      <section className="lp-section">
        ...
        <ul className="lp-limits">
          <li>
            ...
            iOS / iPadOS の Safari と Firefox は Web Serial / Web Bluetooth
            のいずれにも対応していません。
          </li>
          <li>
            <strong>ページを読み込むたびにデバイス選択が必要。</strong>
            Web Bluetooth / Web Serial の許可はセッション単位なので、
          ...
          <li>
            ...
            未対応の FW では UNSUPPORTED_FEATURE になります。BLE 経路は
            従来どおり動きます。
          </li>
          <li>
            <strong>BLE でつなぐとプロファイルを 1 枠使います。</strong>
            空きがない場合はどれかを解放してください（USB では不要）。
          </li>
          <li>
          ...
          <li>
            <strong>BLE の RPC 同期は保証されません。</strong>
            ZMK の RPC は BLE の INDICATE で 20 バイトずつ往復するため、
            キーマップのような大きな応答はブラウザ経由だと数十秒かかり、
            環境によっては完了しません。USB にはこの制限がありません
            （20 バイトの上限は BLE 固有のものです）。どちらの経路でも
            <strong>JSON インポートが確実な代替</strong>です。
          </li>
          <li>
