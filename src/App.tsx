...
export function App() {
  ...
  // USB: the OS already knows every attached port, so this is a plain lookup —
  // no scanning state, no radio, no timeout.
  const refreshPorts = useCallback(async () => {
    setError("");
    try {
      const found = await listSerialPorts();
      setDevices(found);
      setSelected(found.length > 0 ? found[0].id : "");
      if (found.length === 0) setError("USB シリアルポートが見つかりません");
    } catch (e) {
      setError(String(e));
    }
  }, []);

  // Switching transports invalidates the picker: BLE device ids and COM port
  // names are not interchangeable. Refill it (USB) or clear it (BLE, which
  // needs an explicit scan). Keyed on `transport` alone so a failed connect
  // attempt leaves the picker intact.
  useEffect(() => {
    setDevices([]);
    setSelected("");
    setError("");
    if (transport === "usb") void refreshPorts();
  ...
  const doConnect = async () => {
    ...
    setConn("connecting");
    try {
      if (transport === "usb") await connectSerial(selected);
      else await bleConnect(selected);
      ...
      setError(String(e));
      // The port may already be open (serial_connect succeeded and it was the
      // SUBSCRIBE that failed — e.g. firmware without tunnel support). Leaving
      // it held would lock torabo-studio out of a keyboard we are not even
      // connected to, so release it before reporting the failure.
      if (transport === "usb") await linkClose().catch(() => {});
      setConn("disconnected");
    }
  };
  ...
  const doDisconnect = async () => {
    ...
    setConn("disconnected");
    // Over USB the port is only released now, so an app that was locked out
    // (torabo-studio) can take it — and our own picker should be re-read.
    if (transport === "usb") void refreshPorts();
  };
    ...
    <div className="app">
      ...
      {conn !== "connected" && (
        <div className="controls">
          <div className="row">
            <div className="seg" role="group" aria-label="接続方法">
              <button
                className={`seg-btn${transport === "ble" ? " seg-on" : ""}`}
              ...
              <button
                className={`seg-btn${transport === "usb" ? " seg-on" : ""}`}
            ...
            <button
              onClick={transport === "usb" ? refreshPorts : scan}
              disabled={conn === "scanning" || conn === "connecting"}
            >
              {transport === "usb"
                ? "再検出"
